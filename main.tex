...
\begin{titlepage}
    \begin{center}
        ...
        \Huge
        \textbf{First Progression Review}
        ...
        \vspace{0.5cm}
        Bahareh Zaghari, Nick Harris, and Martyn Hill
...
Nozzle guide vanes (NGVs) are static components found in the turbine section of a jet engine (Figure \ref{fig:crossngv}). Temperature monitoring of these components is important for a number of reasons: identifying potential failures before they occur, evaluating the need for maintenance, investigating ways of improving engine efficiency, and reducing fuel consumption. Online temperature monitoring of NGVs is difficult to achieve with currently available technologies (see chapter \ref{literev}). Acoustic based monitoring methods offer a potential alternative. The velocity of acoustic waves is affected by temperature (among other properties) and can be used for temperature monitoring if a suitable sensor system can be developed and implemented. Transducers can be used to transmit acoustic waves through a material and analyse the received signal. The transducers can be placed away from harsh conditions (e.g. turbine gas paths) which would otherwise limit their operation. In the case of nozzle guide vanes (NGVs) their geometry allows for the propagation of Lamb waves at ultrasound frequencies. These waves are dispersive and many distinct modes can be present, which makes their analysis difficult. Despite this problem they can provide high temperature sensitivities (mode dependant) and can travel large distances with limited attenuation. Generation of these waves is possible with a number of different transducer configurations, but the choice is limited by the harsh conditions of the turbine and by space and power constraints. Two potential solutions are the use of waveguides to further distance transducers from the harsh environment, or the use of Piezoelectric Wafer Active Sensors (PWAS) which can be used at high temperatures with careful piezoelectric material selection. The use of either of these methods would make it difficult to selectively excite single modes, but the use of higher order modes could be a potential solution. 

\newpage

At this stage in the study the application of transducers to an NGV is being ignored. The focus is on developing a system that can transmit Lamb waves through a plate and measure the transmitted wave velocity. Measurement of wave velocity to a high degree of accuracy will allow the effect of temperature to be investigated. The system is currently capable of exciting the $S_0$ mode and measuring its time of flight (ToF), see section \ref{results}. Experimental results are being compared to theoretical dispersion curves to validate the accuracy of the system. Wedge transducers are being used for selective Lamb wave mode excitation (currently $S_0$), which simplifies signal analysis. The next stage of the study will investigate the effect of temperature on higher order modes (such as $A_1$, $S_1$, etc.), both experimentally and theoretically, in order to determine which are most suitable for use in a temperature monitoring application to be used at high temperatures. At this stage the development of COMSOL models may aide in understanding the propagation of waves through the wedges and into the plate. Models can also be used to understand the effect of temperature on the system. In order to map temperature across the surface of an NGV (a requirement based on the currently available monitoring systems) it may be possible to use the acoustic reflections caused by the presence of cooling holes found across the surface of NGVs. This will be investigated by introducing holes into the test plate and analysing the reflections when a Lamb wave is excited. Unfortunately the use of wedge transducers is unlikely to be suitable for use on NGVs as the wedge material is required to have a low longitudinal wave speed (lower than the substrate's) which limits the choice of materials to those with a low melting point (such as perspex). A liquid couplant is also required. Further investigation will determine a more suitable transducer configuration, however none of the available options are likely to be able to selectively excite Lamb wave modes in the same way as wedge transducers, which has the potential to complicate signal analysis. One solution to this problem is to target higher order modes, in such a way as to excite a ``cluster" that travels as a single mode. Based on the outcomes of these investigations it may be possible to develop a test system capable of being installed and used on an NGV. In this case the system will be tested at high temperatures in order to validate the system. A plan and gantt chart detailing these aims can be found in section \ref{plan}.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{turbine plus ngv.png}
    \caption{Jet engine cross-section with NGV.}
    \label{fig:crossngv}
\end{figure}
...
The overarching research objective is to develop an online temperature monitoring system for jet engine nozzle guide vanes. This includes signal generation, data acquisition, and a signal processing system. Ideally the system will be capable of monitoring temperature with a resolution, accuracy, and response time, comparable with traditional temperature monitoring systems. This requires the following challenges to be addressed:

\subsection{Up to 18 month review}
\begin{itemize}
    \item Evaluation of the currently used methods for temperature monitoring of NGVs/turbine blades, with a focus on online methods. The outcome of this study can provide the requirements for development of a new system. This is addressed in section \ref{tempmonitoringmethods}.
    \item Determine whether ultrasonic methods are a suitable alternative to the currently available monitoring systems, based on a number of factors such as: response time, accuracy, reliability, power consumption, and possible sensor installation methods. This is addressed in section \ref{ultrasonicmonitoring}.
    \item Identifying a method of exciting Lamb waves in plate-like structures. This is addressed in section \ref{wavegen}.
    \item Understanding the effect of temperature on Lamb wave propagation. This is addressed in section \ref{lambwavesensing}.
    \item Verification of the theoretical temperature sensitivities of Lamb wave modes with experimental data. Measurements up to 160\si{\degreeCelsius} in an aluminium plate. Measurements to be extended to higher order modes. This is addressed in section \ref{lambtheory}.
    \item COMSOL modelling of the system. This will allow the effect of temperature on each element of the system to be predicted. The model can be developed over time to include features such as cooling holes and other transducer configurations.
\end{itemize}    
\subsection{From 18 month review onwards}
\begin{itemize}
    \item Investigating a method of mapping temperature across the surface of an NGV. It is suspected that the numerous cooling holes found across the surface of an NGV would provide acoustic reflections. The varying time of flight between each reflection could be used to monitor temperature at multiple locations. This will be investigated experimentally by introducing holes into a test plate, and modelled using COMSOL.
    \item Development of a transducer configuration suitable for use at high temperatures (see potential options in section \ref{wavegen}). Based on further research a new set of transducers will be tested to verify their ability to send/receive Lamb waves and monitor a change in temperature. Moving away from the use of wedges will allow operation at higher temperatures but will limit the ability to excite single modes. This is likely to complicate signal analysis and so further signal processing development will be required.
\end{itemize}

\section{Research output}

\begin{itemize}
    \item Conference paper ``Temperature measurement using Lamb waves for structural health monitoring at elevated temperatures" submitted to \href{http://www.ewshm2020.com/program/}{European Workshop on Structural Health Monitoring (EWSHM), Palermo, Italy.} Postponed until 2022.
    \item Student poster ``Temperature Monitoring Of Nozzle Guide Vanes (NGVs) Using Ultrasonic Guided Waves" submitted to \href{https://event.asme.org/Turbo-Expo}{ASME Turbo Expo 2020.} Virtual conference 21--25 September 2020.
    \item Journal review paper ``Surface temperature condition monitoring methods for aerospace turbomachinery" submitted (at peer review stage) to \href{https://iopscience.iop.org/journal/0957-0233}{IOP Measurement, Science, and Technology.} To be included in special feature on instrumentation for gas turbines.
    \item Conference paper ``Temperature measurement using ultrasonic guided waves for the structural health monitoring of jet engine nozzle guide vanes (NGVs) at elevated temperatures" submitted to \href{https://conference.isabe.org/}{ISABE2021}, Ottawa, Canada.
\end{itemize}

\chapter{Literature Review} \label{literev}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{tempmethodsvenn.eps}
    \caption{Temperature monitoring systems for nozzle guide vanes}
    \label{fig:venn}
\end{figure}
...
\subsubsection{Thermal paint or Temperature indicating paint} \label{thermalpaint} 
...
\begin{figure}[!htbp]
    ...
    \caption{Calibration colour maps for single-change paints: SC155, SC240, SC275, SC367, SC400, SC458, SC550, SC630 \cite{Neely2006}.}
...
\begin{figure}[!htbp]
    ...
    \caption{Calibration colour maps for multi-change paints: MC135-2, MC165-2, MC395-3, MC490-10, MC520-7 \cite{Neely2006}.}
...
\subsubsection{Thermal history sensors} 
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{crystal temp sens.png}
    \caption{Crystal temperature sensors installed on a NGV \cite{Annerfeldt2004}}
    \label{fig:crystal}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{tftcNGV.png}
    \caption{TFTC array installed on a NGV \cite{Duan2020}}
    \label{fig:tftcngv}
\end{figure}
...
Despite the improvement of thin film thermocouples over conventional thermocouples they are still an invasive monitoring method. Only point measurements are possible unless installed in dense arrays which can have an impact on blade operation, requiring complex wiring systems. The connection between thin-film and leadwires is the primary failure point when installed on turbine blades \cite{Lei1997}. Oxidation has a significant impact on the operation of TFTCs at high temperatures, causing drift and delamination \cite{Guk2016}, which makes the use of an insulation material vital.
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{kerrpyro.eps}
    \caption{Cross sectional view of pyrometer installation \cite{Kerr2004}}
    \label{fig:crosspyro}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{siemens IR edit-01.png}
    \caption{IR camera in use on a Siemens SGT-750 gas turbine \cite{Hellberg}}
    \label{fig:IRsiemens}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{yaneztempTBC.eps}
    \caption{A comparison of online monitoring systems (reproduced from Yañez Gonzalez \textit{et al.} \cite{YanezGonzalez2015})}
    \label{fig:yaneztemp}
\end{figure}
...
\subsection{Ultrasonic Structural Health Monitoring} \label{ultrasonicmonitoring}
...
\subsubsection{Measurement devices for the generation and acquisition of guided waves} \label{wavegen}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{SAW delay line.png}
    \caption{A SAW delay line \cite{Ida2014}}
    \label{fig:sawdelay}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{SAW resonator.png}
    \caption{A SAW resonator \cite{Ida2014}}
    \label{fig:sawresonator}
\end{figure}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{willschwaveguide.png}
    \caption{Willsch's welded waveguide \cite{Willsch2004a}.}
    \label{fig:willsch}
\end{figure}
...
\subsection{Introducing Lamb waves for temperature sensing} \label{lambwavesensing}
...
\begin{figure}[!htbp]
    ...
    \caption{Lamb wave phase velocity dispersion curves for Inconel 718.}
...
In order to evaluate the suitability of an ultrasonic based temperature monitoring system the following study has been carried out:

\begin{itemize}
    \item A 1mm thick aluminium plate was chosen as an initial test material.
    \item Two piezoelectric transducers and two variable angle wedges were purchased. The variable angle wedges allow modes to be selectively targeted.
    \item Dispersion curves for aluminium were produced to determine the frequency and wedge angle required to excite specific Lamb wave modes, and to predict the change in wave velocity with temperature.
    \item A signal generation and data acquisition system was developed to measure the time of flight (ToF) through the plate, and calculate velocity based on a measured distance through the plate.
    \item The temperature of the plate has been lowered using freeze spray and the change in ToF recorded. Temperature has been monitored using thermocouples.
    \item In future the system will be used to measure a change in temperature controlled by a hot plate. Development of this system is ongoing.  
\end{itemize}

\newpage

\section{Theoretical study} \label{lambtheory}

Theoretical dispersion curves based on the material properties of Aluminium have been calculated using \href{https://www.dlr.de/zlp/en/desktopdefault.aspx/tabid-14332/24874_read-61142/#/gallery/33485}{The Dispersion Calculator}. Curves have been produced from room temperature up to 160\si{\degreeCelsius} by varying material properties (Figure \ref{fig:grouptempshuftalu}). The change in density (thermal expansion) and poisson’s ratio is negligible in comparison to the change in young’s modulus. Thermal expansion will cause a change in material dimensions which will also affect the wave speed, but the effect of this is small ($-$1.19 m s$^{-1}$ average change over 20--160\si{\degreeCelsius}). The change in Young’s modulus with temperature in Aluminium is close to linear, as represented by the equation \cite{Hopkins2012}: 
%
\begin{equation} 
...
The values of Young’s modulus produced by this equation have been used to generate temperature dependant dispersion curves for aluminium (Figure \ref{fig:alutempshift}). The group velocity of the $S_0$ mode at 1 MHz has been extracted from the curves and used to plot figure \ref{fig:alu1mhz}. Calculated temperature sensitivity of the mode is 1.671--1.769 m s$^{-1}$\si{\degreeCelsius}$^{-1}$ over the temperature range 0--160\si{\degreeCelsius} (see equation \ref{eqn: eq k}). This value is frequency and mode dependent. The temperature sensitivity of the $S_0$ mode increases with temperature, as the mode reduces in frequency (shifts left), and reduces in wave velocity (shifts down), the frequency-thickness product of 1 MHz-mm falls into a more dispersive region.

Time of flight (ToF) can be calculated from the equation \cite{Croxford2007a}:
%
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{grouptempshiftalu.eps}
    \caption{$A_0$, $S_0$, $A_1$, and $S_1$ group velocity dispersion curve shift with temperature from 20\si{\degreeCelsius} to 160\si{\degreeCelsius} for Aluminium 1050 H14.}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{grouptemp1mhztempalu.eps}
    \caption{$S_0$ group velocity change with temperature from 20\si{\degreeCelsius} to 160\si{\degreeCelsius} at 1 MHz for Aluminium 1050 H14.}
    \label{fig:alu1mhz}
\end{figure}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=\columnwidth]{main setup.jpg}
    \caption{Test setup.}
    \label{fig:testsetup}
\end{figure}

The $S_0$ Lamb wave mode has been excited in an aluminium plate using a piezoelectric transducer attached to an acrylic wedge (Olympus variable angle wedge), which is coupled to the plate with a liquid couplant (Figure \ref{fig:testsetup}). The wedge allows careful selection of excitation angle so that only the $S_0$ mode is excited. The angle is determined based on Snell’s law:
%
\begin{equation} 
...
\end{equation} 
%
The $A_0$ mode cannot be excited using this method as it’s phase velocity at this frequency (2312 m s$^{-1}$) is slower than the longitudinal velocity of the wedge. If the $A_0$ mode is present in the signal it will not affect measurement of the $S_0$ mode as it’s group velocity is significantly different than that of the $S_0$ mode, which will cause a distinct second wave packet. A signal generator (Hewlett Packard 33120A) has been used to generate a 5-cycle tone burst at 1 MHz, which is transmitted through the plate and received by an identical transducer and wedge in a pitch-catch configuration. Time of flight (ToF) is measured between transducers using a cross-correlation function. Wave velocity is calculated from the distance between transducers. The propagation time through the wedges has been subtracted from the total ToF to ensure that only the propagation time through the plate is measured. Based on a sampling rate of 5$\times$10$^8$ the theoretical maximum temporal resolution is 2 ns. This is equal to a velocity resolution of $\pm$0.35 m s$^{-1}$.
\\
In future the temperature of the aluminium plate will be controlled using a hot plate (figure \ref{fig:testdiagram}) and raised from room temperature to 160\si{\degreeCelsius} (just below the melting point of the wedges). The temperature of the plate will be monitored using thermocouples. This system needs further work to eliminate the effect of temperature on the perspex wedges, which is causing a much larger change in calculated wave velocity than expected. To demonstrate the capability of the system a different test has been carried out. The temperature of only the plate has been reduced using freeze spray applied to the area between wedges.

\begin{figure}[!htb]
    \centering
    \includegraphics[width=\columnwidth]{testdiagram.eps}
    \caption{Diagram of test setup and $S_0$ mode wave propagation.}
    \label{fig:testdiagram}
\end{figure}

\subsection{Results}

Figure \ref{fig:freeze} shows the change in ToF as the temperature of the aluminium plate is lowered from room temperature (23\si{\degreeCelsius}) down to between -10\si{\degreeCelsius} and -35\si{\degreeCelsius} using freeze spray, and then allowed to increase back to room temperature. The distance between wedges is 0.1 m. Temperature is measured in two locations, under the plate and on top of the plate, both equidistant from the wedges. Subtracting the ToF through the wedges (31.246 \si{\micro\second}) from the overall ToF (57.616 \si{\micro\second}) at room temperature (23\si{\degreeCelsius}), the wave velocity can be calculated from equation \ref{tofcalc}, where $d$ is equal to 0.133 m (0.1 m + the additional distance travelled under each wedge). The wave velocity in this case is 5043.61 m s$^{-1}$. This corresponds to the predicted wave velocity of the $S_0$ mode as seen in figure \ref{fig:alu1mhz}.

\begin{figure}[!htb]
    \centering
    \includegraphics[width=\columnwidth]{freeze.eps}
    \caption{Time of flight change with temperature when aluminium plate is cooled with freeze spray.}
    \label{fig:freeze}
\end{figure}

\section{Conclusion}

The theoretical effect of temperature on various Lamb wave modes in aluminium plates has been investigated by generating dispersion curves based on varying material properties (figure \ref{fig:alutempshift}). This can be repeated in the future for other materials at higher temperatures (e.g. Inconel 718 up to 1100\si{\degreeCelsius} in figure \ref{fig:groupshift}). The temperature sensitivity of the $S_0$ mode at 1 MHz has been extracted from these curves (figure \ref{fig:alu1mhz}) to be validated experimentally.
\\
Wedge transducers in a pitch-catch configuration have been used to excite the $S_0$ mode in a 1mm thick aluminium plate. The time of flight between transducers has been measured using a cross-correlation method and wave velocity calculated based on the distance between transducers. This confirms that the $S_0$ mode has been excited. The effect of lowering the temperature of the plate using freeze spray can be seen in figure \ref{fig:freeze}. Further experiments will attempt to validate the temperature sensitivity of the $S_0$ mode against theoretical values extracted from dispersion curves.
...
Although there is a measurable ToF change with temperature the result is a combination of wave speed change in the aluminium plate, and a wave speed change in the perspex wedges. Subtracting the ToF of the wedges should leave only the ToF through the plate, but because this time is temperature dependant it is proving difficult to accurately measure or calculate. One solution to this problem is to change the distance between wedges and measure the ToF multiple times at different distances. The change in ToF would only be due to distance change in the plate, eliminating the effect of the wedges. This requires precise and repeatable distances between transducers to be maintained, which can be achieved with rigid spacers.

\subsection{Long term plans}

\begin{itemize}
    \item The next target is to produce results suitable for publication (submission deadline for ISABE2021 is March 2021). This includes utilisation of the $S_0$ mode for temperature monitoring up to 150\si{\degreeCelsius} (below perspex wedge melting point) in an aluminium plate, and theoretical calculation of mode sensitivities based on material properties for validation. 
    \item Frequency/thickness product can be raised to excite higher order modes (or HOMCs) which could be appropriate when using more broadband transducers (e.g. PWAS). This can be achieved by increasing the thickness of the plate or by using transducers at higher frequencies.
    \item As temperature is further increased it will have more of an effect on the operation of transducers. The effect on amplitude and frequency can be investigated.
    \item Modelling of the system (COMSOL) would allow for a better understanding of how temperature is affecting the transducer, wedge, and plate. Additional models can be produced as the system develops.
    \item Reflection points (holes) can be introduced to the aluminium plate in order to map temperature at a number of locations. Time of flight can be measured between each reflection. This will mimic the cooling holes found on NGVs. The results of this experiment can be included in a subsequent publication (to submit mid 2021?). 
    \item To operate at higher temperatures a new transducer configuration will be required. Piezoelectric Wafer Active Sensors (PWAS) using piezoelectric materials such as YCOB or AlN (that can operate at extremely high temperatures) are a potential option. Another option is to distance the transducers away from the high temperatures using waveguides, and couple into the test structure using a system of Hertzian contact points. Both options require further investigation. 
\end{itemize}

\subsection{Gantt chart}

\includepdf[landscape=true,fitpaper=true]{Gantt_Chart}
